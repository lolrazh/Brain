...
def send_request(user_prompt, Description, API_Key):
    ...
    url = "https://api.openai.com/v1/chat/completions"  # reference gpt model
    ...
    request.Headers.Add("Authorization", "Bearer " + str(API_Key))
...
class Brain(component):
    def __new__(cls):
        instance = Grasshopper.Kernel.GH_Component.__new__(cls,
            "Brain", "Brain", """Uses GPT to manipulate Grasshopper scripts through natural language.""", "Params", "Util")
        return instance
    ...
    def RunScript(self, Parameters, API_Key, Description, Trigger):
        ...
        def change_slider_value(nickname, value):
            ...
            for obj in ghdoc.Objects:
                if isinstance(obj, Grasshopper.Kernel.Special.GH_NumberSlider) and obj.NickName == nickname:
                    if obj.Slider.Minimum <= value <= obj.Slider.Maximum:
        ...
        if not ghdoc:
            ...
            input_params = self.Params.Input[0].Sources if self.Params.Input else []
            param_data = [(param.NickName, param.CurrentValue) for param in input_params if isinstance(param, Grasshopper.Kernel.Special.GH_NumberSlider)]
        if param_data:
            if Trigger:
                prompt = construct_prompt(param_data)
                if prompt is not None: # avoid running when user hits cancel
                    response = send_request(prompt, Description, API_Key)
